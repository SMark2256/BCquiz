...
import { VoteDoneBox } from "@/components/features/vote-done-box";

export function VotingWidget() {
  const { topics, loading, error, vote, hasVoted, hasVotedAny, votedTopic } =
    useVoting();
  ...
  return (
    <div className="overflow-hidden rounded-xl border-2 border-white/10 bg-foreground p-1">
      ...
      {hasVotedAny ? (
        <VoteDoneBox votedTopic={votedTopic} />
      ) : (
        <>

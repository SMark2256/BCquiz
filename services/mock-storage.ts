  ...
  const defaultQuizzes: Quiz[] = [
    {
      ...
      category: "Sorozat",
      imageUrl:
        "https://image.tmdb.org/t/p/w500/V2UjGqe24QJJHyEChd4D1AxmOB.jpg",
      isActive: true,
      id: "mock-1780345661264",
      createdAt: new Date("2026-07-01T20:27:41.264Z"),
      updatedAt: new Date("2026-07-01T20:27:41.264Z"),
    },
    {
      title: "The Witcher",
      titleHu: "Vaják",
      description:
        "Ríviai Geralt, a felbérelhető, mutálódott szörnyvadász végzete felé sodroudik egy vészterhes világban, ahol az emberek gyakran gonoszabbak a szörnyeknél.",
      date: new Date("2026-07-10T00:00:00.000Z"),
      time: "20:00",
      location: "BarCraft Corvin",
      category: "Sorozat",
      imageUrl:
        "https://image.tmdb.org/t/p/w500/qJU4px38JAEF4iN5sh41EkO2n7x.jpg",
      isActive: true,
      id: "mock-1780345667603",
      createdAt: new Date("2026-07-01T20:27:47.603Z"),
      updatedAt: new Date("2026-07-01T20:27:47.603Z"),
    },
  ];

  storage.setItem(STORAGE_KEYS.QUIZZES, JSON.stringify(defaultQuizzes));
  storage.setItem(
    STORAGE_KEYS.VOTING_SESSIONS,
    JSON.stringify(getDefaultVotingSessions()),
  );
...
export function toggleLocalQuizActive(id: string): ApiResponse<Quiz> {
  ...
  const updatedQuiz = {
    ...quizzes[index],
    isActive: !quizzes[index].isActive,
    updatedAt: new Date(),
  };
  quizzes[index] = updatedQuiz;
  ...
  return [
    {
      ...
      votepool: [
        ...
        },
        {
          ...
          imageUrl:
            "https://image.tmdb.org/t/p/w500/9RQhVb3r3mCMqYVhLoCu4EvuipP.jpg",
          votes: 31,
        },
      ],
      createdAt: now,
...
export function createLocalVotingSession(
...
export function updateLocalVotingSession(
  ...
  try {
    ...
    const updated: VotingSession = {
      ...
      votepool: data.votepool
        ? buildVotepool(data.votepool, existing.votepool)
        : existing.votepool,
      updatedAt: new Date(),
    };
...
export function setLocalActiveVotingSession(
  id: string,
): ApiResponse<VotingSession> {
  const sessions = getLocalVotingSessions();
...
export function toggleLocalVotingSessionActive(
  id: string,
): ApiResponse<VotingSession> {
...
export function voteLocalVoteTopic(
  sessionId: string,
  topicId: string,
  fingerprint: string,
): ApiResponse<VotingSession> {
...
export function resetLocalVotingSessionVotes(
  sessionId: string,
): ApiResponse<VotingSession> {
  const storage = getStorage();
  if (!storage) {
